...
const int numLeds = 14; // How many neopixels in the string? used to start the NeoPixel library
...
uint8_t myPass = 0;   //  how many times have we cycled through the main loop?
uint8_t myPrevious = 3;   // what was the last BaseColor used?
uint8_t mode = 0;  // this variable maps to the sequence that we WANT to select
int sequence = 1;  //  which sequence number is up next?
uint8_t myPreviousChoice = 5;  // what was the last sequence number picked?
uint8_t pixelCount = numLeds;  // how many pixels, in a pixel-based effect?
int loopCount = 1;  //how many cycles for word-based effects?
uint16_t nameDelay = 1500;  // name-delay is longer delay, usually between effect patterns
uint8_t icDelay = 250;  // inter-character delay, for pauses between transitions
uint8_t ipDelay = 60;  // inter-pixel delay, so you can see the wipe.fade patterns move quickly
uint8_t myBase = 0;
uint8_t lpixel = 0;
uint8_t hpixel = (numLeds);
int cycles = 1;
int speed = 15;
uint8_t pick = 5;
...
void loop() {
  ...
  if (sequence == 2) {
    ...
    int speed = random(15,80);  // How slow should the colors shift? Higher number = slower
    ...
    int cycles = 1;
    if (mode == 1) { cycles = random(4,14); } // roll the dice, 2-7 repeats 
    ...
    int pixels = random(40,130);  // How many pixel changes? Pick a large range (low, high)
    randomPixels(700, pixels); // delay between changes, how many pixels to change
    }
  else if (sequence == 6) {  
    /* This sequence 'wipes' all the pixels, one at a time, with a color, but it leads
    ...
    int pause = (2000 * (random(2,9)) );  //  Add a pause to enjoy the effect
    wipeColorM2(cycles, pause); 
    }
  else if (sequence == 9) { 
    if (mode == 1) { cycles = random(2,9); } // roll the dice, 1-6 repeats 
